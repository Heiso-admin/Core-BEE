    ...
    <div className="container mx-auto pt-4 pr-4 h-full flex flex-col">
      ...
      <div className='flex items-center justify-between mb-4'>
        ...
        <div className="flex gap-2">
          ...
          >
            <Plus className="h-4 w-4" />{t("list.add_new")}
          </Button>
        </div>
      </div>
      <div className='space-y-4'>
        {data.map((role) => (
          <RoleItemCollapsible key={role.id} role={role} menus={menus} permissionGroups={permissions} isCollapsedClose={isCollapsedClose} />
        ))}
      </div>
    </div>
  );
...
function RoleItemCollapsible(
  ...
  const [selectedMenus, setSelectedMenus] = useState<string[]>(role.menus?.map((m: any) => m.menus.id) ?? [])
  ...
  const ItemColStyle = { gridTemplateColumns: 'minmax(200px, 1fr) 3fr', gap: "1rem" }

  // helpers: 取得角色預設的選單/權限 ID（穩定引用）
  const getRoleMenuIds = useCallback((r: Role) => r.menus?.map((m: any) => m.menus.id) ?? [], [])
  const getRolePermissionIds = useCallback((r: Role) => r.permissions?.map((p: any) => p.permission.id) ?? [], [])

  // helpers: 將狀態重置為角色目前資料（不改 isEditing）（穩定引用）
  const applyRoleState = useCallback((r: Role) => {
  ...
  const snapshotPrevIfNeeded = useCallback(() => {
    if (!prevPermissionsRef.current) prevPermissionsRef.current = selectedPermissions
  ...
  const allPermissionIds = useMemo(() => {
    const ids: string[] = []
      ; (permissionGroups || []).forEach((g: any) => {
        ; (g.permissions || []).forEach((p: any) => { ids.push(p.id) })
      })
  ...
  }, [snapshotPrevIfNeeded, selectedPermissions.length, allPermissionIds, selectedMenus.length, allMenuIds])
        ...
        <div className="grid items-center justify-between" style={ItemColStyle}>
          <div className="flex items-center gap-2 ">
            ...
            <label
              ...
              className={cn("text-sm ml-2", !isEditing ? "text-muted-foreground" : "")}
            >
              <h5 className="text-sm font-medium text-muted-foreground">{t("form.menuTitle")}</h5>
            </label>
          ...
          <h5 className="text-sm font-medium text-muted-foreground">{t("form.permissionTitle")}</h5>
        </div>
